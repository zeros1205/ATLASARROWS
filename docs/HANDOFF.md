...
1. **런칭 블로커 (코드 아님)**: AdMob 실계정 ID, 스토어 IAP 상품 등록, 앱 아이콘/스플래시,
   릴리즈 서명, 스토어 스크린샷
2. 앱 아이콘·스플래시 제작 (요청 시 바로 진행 가능)
3. 실기기 QA: 햅틱 체감, 광고 타이밍, 레벨 3~10 난이도 곡선(1분+ 체감 확인)
4. 밀린 것들: 한글 UI 폰트 번들(현재 영문 UI), 레벨 JSON 프리베이크, 실루엣 438행 회수(보류),
   데일리 챌린지(보류), 픽처 보스 마스크(보류)
