...
- **픽처 실루엣 파이프라인 (d6dc838)**: 지난 세션 말미에 `validate_shapes.py`가 GPT 픽처
  ...
  붙이고, `validate_shapes.py`는 기존 파일에 병합(같은 이름은 교체).
  ⚠️ 이후 사용자 판정: **GPT 픽처 10개는 품질 미달로 불채택** → batch_01.txt 삭제, 카탈로그
  1,562개로 확정. 파이프라인은 남아 있으니 좋은 픽처가 생기면 shapes_raw/에 넣고 재빌드만
  하면 됨. GPT에게 픽처 실루엣을 다시 시키는 건 사용자 승인 없이 하지 말 것.
