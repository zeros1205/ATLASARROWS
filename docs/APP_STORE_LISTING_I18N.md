...
> 이 개정 대상이 아니다.
>
> **같은 날 4차 개정 — 중복 제거.** Apple 공식 ASO 가이드: 앱 이름·서브타이틀·
> 카테고리에 이미 있는 단어는 자동 인덱싱되므로 키워드에 다시 넣어도 검색 가중치가
> 없다. 로케일마다 (a) 브랜드명 `Atlas Arrows: Tap Puzzle`(전 로케일 공통, 번역 안 함),
> (b) 그 로케일의 서브타이틀, (c) 카테고리(게임/퍼즐)와 겹치는 단어를 제거했다.
> 예: EN에서 `puzzle`(카테고리)·`arrow`/`arrows`(브랜드명+서브타이틀)·`tap`(브랜드명+
> 서브타이틀)·`atlas`(브랜드명+서브타이틀)·`game`(카테고리 "Games")을 뺐다. FR/ES/PT
> 키워드에 있던 영어 차용어 `tap`도 전 로케일 공통 브랜드명 "Tap Puzzle"과 겹쳐 뺐다.
> 단수/복수가 겹치는 경우(예: `pfeil`/`Pfeile`)는 서브타이틀 쪽 단어로 이미 커버되니
> 키워드에서는 통째로 뺐다 — 억지로 단수형만 남기지 않았다.
>
...
### Keywords
```
brain,casual,map,offline,nowifi,logic,teaser,iq,mind,travel,maze
```
...
### キーワード
```
矢,脳,カジュアル,地図,オフライン,ノーワイファイ,ロジック,ティーザー,IQ,マインド,旅,迷路
```
...
### Schlüsselwörter
```
gehirn,casual,karte,offline,logik,teaser,verstand,reisen,labyrinth
```
...
### Mots-clés
```
cerveau,casual,carte,horsligne,logique,teaser,esprit,voyage,labyrinthe
```
...
### Palabras clave
```
cerebro,casual,mapa,offline,logica,teaser,ci,mente,viaje,laberinto
```
...
### Palavras-chave
```
cerebro,casual,mapa,offline,logica,teaser,qi,mente,viagem,labirinto
```
...
| 키워드 | `화살표,두뇌,캐주얼,지도,아틀라스,오프라인,와이파이없음,로직,티저,아이큐,마인드,여행,미로` |
