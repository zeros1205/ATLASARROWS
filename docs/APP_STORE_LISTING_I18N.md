...
### Keywords
```
puzzle,arrow puzzle,escape game,brain training,world map,casual,offline,logic game
```
...
### キーワード
```
パズル,矢印パズル,脱出ゲーム,脳トレ,世界地図,カジュアル,オフライン,ロジックゲーム
```
...
### Schlüsselwörter
```
puzzle,pfeilpuzzle,fluchtspiel,gehirntraining,weltkarte,casual,offline,logikspiel
```
...
### Mots-clés
```
puzzle,flèche,jeu d'évasion,cerveau,carte du monde,casual,hors ligne,jeu de logique
```
...
### Palabras clave
```
puzzle,flecha,juego de escape,cerebro,mapamundi,casual,sin conexión,juego de lógica
```
...
### Palavras-chave
```
puzzle,seta,jogo de fuga,cérebro,mapa múndi,casual,offline,jogo de lógica
```
...
| 키워드 | `퍼즐,화살표퍼즐,탈출게임,두뇌트레이닝,세계지도,캐주얼,오프라인,로직게임` |
