...
```
화살표,두뇌,캐주얼,지도,아틀라스,오프라인,와이파이없음,로직,티저,아이큐,마인드,여행,미로
```

> 2026-07-24 3차 개정 — 사용자가 직접 제시한 EN 키워드 세트(`puzzle,arrow,arrows,
> brain,casual,map,atlas,tap,offline,nowifi,logic,game,teaser,iq,mind,travel,maze`,
> 99/100자)를 기준으로 전 언어 재작성. 이전 개정보다 단어 폭을 넓혀 검색 노출 범위를
> 키운 것 — `teaser`·`iq`·`mind`(두뇌 자극 계열), `travel`·`maze`(테마 계열), 그리고
> 브랜드 자체인 `atlas`를 키워드로도 넣었다.
>
> **4차 개정(같은 날) — 중복 제거.** Apple 공식 가이드: **앱 이름·서브타이틀·카테고리에
> 이미 있는 단어는 키워드에 다시 넣어도 검색 가중치가 붙지 않는다**(인덱싱이 자동으로
> 되기 때문). `퍼즐`(카테고리 "게임 › 퍼즐"과 중복), `탭`·`화살`(서브타이틀 "화살
> 퍼즐을 탭하고..."와 중복 — "화살표"는 서브타이틀에 없는 별개 단어라 유지), `게임`
> (카테고리 "게임"과 중복)을 뺐다. 다른 6개 언어도 각 로케일의 서브타이틀·카테고리와
> 겹치는 단어를 같은 기준으로 뺐다 — 전체는 `docs/APP_STORE_LISTING_I18N.md` 참고.
