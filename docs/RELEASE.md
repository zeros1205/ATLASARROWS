...
##### ⛔ 함정 4 — 러너 이미지의 Xcode가 낡으면 **업로드 직전에** 거부당한다

App Store Connect는 **iOS 26 SDK 미만으로 빌드된 것을 거부**한다. `macos-15`는
Xcode 16(iOS 18.5 SDK)이라, 아카이브까지 다 성공한 뒤 마지막 업로드에서 409로
튕긴다 — 가장 늦게 알게 되는 방식이다.

→ `runs-on: macos-26`. 워크플로가 시작 직후 `xcodebuild -version`을 찍으므로,
다음에 SDK 요구가 또 올라가면 전체 빌드를 기다리지 않고 바로 알 수 있다.

##### ⛔ 함정 5 — IPA 파일명을 하드코딩하지 말 것

Flutter는 IPA를 **앱 표시명**으로 만든다(`Atlas Arrows.ipa`). Dart 패키지명이
아니다. 워크플로는 `ls build/ios/ipa/*.ipa`로 실제 산출물을 찾는다.
**앱 이름을 바꿔도 안 깨진다.**

##### ⛔ 함정 6 — 서비스 계정 JSON은 base64로 넘길 것

JSON을 시크릿에 그대로 담으면 BOM·CRLF가 섞여 개인키 PEM이 깨지고, Firebase CLI가
**`Failed to authenticate, have you run firebase login?`** 이라는 **권한 문제처럼
보이는 메시지**를 낸다(실제로는 자격증명 손상). 로컬에서 같은 키로 인증이 되는지
확인하면 구분된다.

→ `FIREBASE_SERVICE_ACCOUNT_BASE64`로 넘기고 워크플로에서 디코딩 + JSON 파싱 검증.

##### ⛔ 함정 7 — CI에는 개발용 인증서가 없다
...
| 시크릿 | 내용 |
|---|---|
| `FIREBASE_ANDROID_APP_ID` | `1:522578851235:android:f53f71f9746d0e9a611616` (google-services.json의 `mobilesdk_app_id`) |
| `FIREBASE_SERVICE_ACCOUNT_BASE64` | 서비스 계정 JSON을 **base64로** (원문 그대로 넣으면 깨진다 — 함정 6) |
...
서비스 계정은 Play용(`play-publisher`)과 분리해 **`firebase-distributor`**를 따로
두고 `roles/firebaseappdistro.admin`만 부여했다. 브라우저 폼은 GCP의 단축키와
충돌해 입력이 자꾸 날아가므로 **`gcloud` CLI가 훨씬 안정적**이다:

```powershell
gcloud iam service-accounts create firebase-distributor --project=atlasarrows-7a720
...
gcloud iam service-accounts keys create D:\Downloads\fad-key.json `
  --iam-account=firebase-distributor@atlasarrows-7a720.iam.gserviceaccount.com
```

테스터 그룹은 `internal`. Firebase 프로젝트 자체 설정은 `docs/FIREBASE.md` 참고.
...
| **TestFlight 실업로드** | ✅ **성공** (run 29689215923, Xcode 26.5 / iOS 26.5 SDK) |
| **Firebase App Distribution** | ✅ **성공** (run 29689211721, `internal` 그룹 배포) |
| Play 업로드 | ⏳ **첫 AAB를 콘솔에서 수동 업로드**해야 API 업로드가 열림 |
| FAD iOS 배포 | ⏳ ad-hoc 프로파일 필요 (App Store 프로파일로는 불가) |
