...
⛔ **배포 워크플로는 사용자가 지시할 때만 실행한다.** FAD·TestFlight·Play 업로드를
자발적으로 트리거하지 말 것. 시크릿이 준비됐다거나 검증이 필요해 보인다는 이유로
돌리는 것도 안 된다 — 먼저 물어볼 것. 상시 승인된 것은 git 커밋·푸시뿐이다.
빌드만 확인하려면 로컬 `flutter build`로 충분하다.
